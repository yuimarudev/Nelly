...
Discord.APIMessage.transformOptions = function(content, options, extra = {}, isWebhook = false) {
    ...
    if (Array.isArray(options)) {
      ...
      return isWebhook ? { content, embeds, files, components: wrapButtonsByActionRows(buttons), ...extra } : { content, embed: embeds[0], files, components: wrapButtonsByActionRows(buttons), ...extra };
    } else if (Array.isArray(content)) {
      const [embeds, files, buttons] = this.partitionMessageAdditions(content);
      if (embeds.length || files.length || buttons.length) {
        return isWebhook ? { embeds, files, components: wrapButtonsByActionRows(buttons), ...extra } : { embed: embeds[0], files, components: wrapButtonsByActionRows(buttons), ...extra };
      }
    }
